...
### `lab/nav.html` — 導覽改造的四個方案

功能超過十五個之後，平鋪的清單就不好找了。這裡有四種「分類 → 細項」的互動：

| 方案 | 怎麼操作 | 適合 |
|---|---|---|
| **輪盤** | 按住分類，功能繞成一圈，往哪個方向滑就選哪個 | 熟手，方向可變肌肉記憶 |
| **拇指扇** | 功能排成扇形往上開，左右擺拇指掃過去 | 單手、邊緣、字多 |
| **滑選清單** | 功能排成一列，上下滑動高亮 | 第一次用、項目多 |
| **兩段輪盤** | 內圈分類、往外拖展開外圈功能，一個手勢走完 | 進階手勢 |

四種共用同一套手勢（Pointer Events，觸控與滑鼠一致），也都留了
**輕點＝靜止選單**的保底路徑 —— 長按加拖曳對手指不方便的人不友善，
沒有這條路徑不能上線。

做原型時發現的事：**手機上排不出整圈輪盤**。圓心必須咬著手指
（否則「手指沒移動」就不等於取消，放開會變誤選），而分類列就在畫面邊上，
左右沒有一個半徑的空間。所以圓環放不下時會自動退化成扇形 ——
換句話說，輪盤在手機上的最終形態就是拇指扇。
